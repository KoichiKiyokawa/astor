...
public class ScaffoldRepairEngine extends AstorCoreEngine {
	...
	public ScaffoldRepairEngine(MutationSupporter mutatorExecutor, ProjectRepairFacade projFacade) throws JSAPException {
	...
	public boolean synthesizeSketch(String sketchtype, ModificationPoint changepoint) throws Exception {
		...
		if (childCompiles) {
			log.debug(
					"-The created sketch compiles: SketchType " + sketchtype + " ModificationPoint " + changepoint.toString());
	...
	public List<String> scaffoldGenerationAll() {
		...
		for (ModificationPoint modificationPoint : modificationPointsToProcess) {
			...
			transformer.add(new OverloadMethodTransform(modificationPoint, modificationPointIndex, this.mutatorSupporter,
					this.projectFacade, this));
			transformer.add(new ConditionRemoveTransform(modificationPoint, modificationPointIndex, this.mutatorSupporter,
					this.projectFacade, this));
			transformer.add(new ConditionMutationTransform(modificationPoint, modificationPointIndex, this.mutatorSupporter,
					this.projectFacade, this));
	...
	public List<String> scaffoldGenerationSpecific(String strategyName, ModificationPoint specificModificationPoint,
		...
		if (strategyName.equals("OverloadMethodTransform"))
			...
			transformer = new OperatorTransform(specificModificationPoint, index, this.mutatorSupporter, this.projectFacade,
					this);
		else if (strategyName.equals("ExpressionTransform"))
			transformer = new ExpressionTransform(specificModificationPoint, index, this.mutatorSupporter, this.projectFacade,
					this);
