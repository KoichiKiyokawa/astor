				...
				Collections.sort(baseElements, new Comparator<Ingredient>() {
					...
					public int compare(Ingredient ingredientA, Ingredient ingredientB) {
						...
						if (ingredientA.commitMessage.length() == 0) {
							...
							// ingredientAのコミットメッセージが空で、
							// ingredientBのコミットメッセージが空じゃなかったら、Aをうしろに
							return 1;
						} else if (ingredientB.commitMessage.length() == 0) {
              // ingredientAのコミットメッセージが空ではなく、
              // ingredientBのコミットメッセージが空だったら、
              return -1;
            }
